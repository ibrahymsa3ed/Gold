...
- **Background price watcher (Android):** `lib/services/price_watcher.dart` registers a periodic `workmanager` task that fetches prices via `GoldScraper`, pushes the latest sell prices to the iOS widget shared store via `home_widget`, and (until the FCM rollout flips) fires a 1-hour throttled local notification with sell-only body for 21K/24K/ounce. Initialized from `main.dart` on Android only.
- **FCM fixed-time push summaries (rolling out):** The backend at `main-backend/` schedules push deliveries to enrolled devices at 07:00, 11:00, 15:00, 19:00 Africa/Cairo. Devices register via `POST /api/devices` with `{ device_id, platform, fcm_token, locale, build_number }` (token de-dup is atomic; `last_sent_slot` survives token rotation). The sweep job (`main-backend/src/notificationsScheduler.js`) runs every 5 minutes via `node-schedule`, computes the active Cairo slot using `luxon` (DST-safe), enforces a 30-minute slot window, and refuses to send when the cached price is older than 30 minutes (one re-sync attempt, then skip — never deliver stale prices). Two layered kill switches gate the rollout: `FCM_SUMMARIES_ENABLED=false` stops the scheduler entirely, and `MIN_FCM_CLIENT_BUILD=999999` rejects every device whose installed app reports `build_number` below the threshold. Bodies are localized — English uses `21K/24K/Ounce`, Arabic uses `عيار 21/عيار 24/الأونصه` with Western digits. Title flips to `أسعار الذهب` in Arabic. Invalid/expired FCM tokens are pruned automatically on send failure.
- **FCM client wiring (Phase 2):** `lib/services/push_notifications_service.dart` is the Flutter-side counterpart to the backend scheduler. On first dashboard build it requests notification permission, fetches the FCM token (and registers a token-refresh listener), generates/loads a stable per-install `device_id` from `SharedPreferences`, reads `build_number` from `package_info_plus`, and calls `POST /api/devices`. The backend response carries `fcm_summaries_active` (= `FCM_SUMMARIES_ENABLED && build_number >= MIN_FCM_CLIENT_BUILD && summaries_enabled`) which is persisted locally; `PriceWatcher`, `IosBackgroundFetch`, and the dashboard's foreground notification all consult `PushNotificationsService.isFcmActive()` and self-disable their local notification firing when set, so users never get double notifications during/after the rollout. The top-level FCM background message handler (`firebaseMessagingBackgroundHandler`) is registered in `main.dart` before `runApp`. Foreground messages are rendered via the existing `NotificationsService` (iOS doesn't auto-show banners while the app is foregrounded). Locale changes in Settings push the new locale to the backend via `PUT /api/devices/:device_id` so summary bodies switch language live, and also write `locale` into the App Group so the iOS widget re-renders karat labels (`21K` vs `عيار 21`) on its next refresh. The Settings section exposes a "Price summaries" switch (mirrors `summaries_enabled`) and a "Send test notification" button that hits `POST /api/devices/:device_id/test`.
